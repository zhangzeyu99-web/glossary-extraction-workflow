...
翻译前增量协作走稳定 `export_glossary_candidates.py` CLI，不再依赖另一个长期任务持续在线。分类器只发现候选，不自动批准入库；普通动作/提示与可组合词不因有类别就成为长期术语，正式专名也不按出现次数砍掉。一次候选价值裁决后把结果交给主控，主控负责术语补译与正文；不要双方各自全量再审一遍。已有术语默认只列本批命中的缺译，完整词典清理另立明确任务。
